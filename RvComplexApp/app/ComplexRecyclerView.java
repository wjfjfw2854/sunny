        ...
        switch (e.getAction())
        {
            ...
            case MotionEvent.ACTION_MOVE:
                if(state == STAT_RESET)
                {
                    ...
                    if(type == RvItemDataType.TYPE_TOP0 && moveX > moveY && moveX > touchSlop)
                    {
                        state = STAT_HORIZONTAL_SCROLL;
                        canScrollMaxX(getMeasuredWidth());
                    }
                    else if(type == RvItemDataType.TYPE_TOP1 && moveX > moveY && moveX > touchSlop)
                    {
                        state = STAT_HORIZONTAL_SCROLL;
                        canScrollMaxX(getMeasuredWidth());
                    }
        ...
        switch (e.getAction())
        {
            ...
            case MotionEvent.ACTION_OUTSIDE:
                if(state == STAT_HORIZONTAL_SCROLL)
                {
                    ...
                    float scrollMaxMid = scrollMaxX;
                    if(type == RvItemDataType.TYPE_TOP0)
                    {
                        curScrollXMid = curScrollX;
                        scrollMaxMid = scrollMaxX;
                    }
                    else if(type == RvItemDataType.TYPE_TOP1)
                    {
                        curScrollXMid = curScrollX1;
                        scrollMaxMid = scrollMaxX1;
                    }
                    ...
                    {
                        mScroller.fling(curScrollXMid,0,-velocityX,0,0,(int)Math.max(0,scrollMaxMid),0,0,scrollLength,scrollLength);
                        invalidate();
                    }
                    else
                    {
